...
def dodaj_artikal():
    ...
    cursor = db.cursor()
    q = '''INSERT INTO
    artikli(naziv, opis, slika, cijena, stanje, kolicina, datum, kategorije_id, korisnici_id)
    VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)'''
    data["datum"] = datetime.datetime.strptime(data["datum"], "%d.%m.%Y. %H:%M:%S")
    cursor.execute(q, (data["naziv"], data["opis"], data["slika"],
                       data["cijena"], data["stanje"], data["kolicina"], data["datum"],
...
def izmeni_artikal(id_artikla):
    ...
    cijena=%s, stanje=%s, kolicina=%s, datum=%s,
