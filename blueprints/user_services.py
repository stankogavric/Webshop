...
def dodaj_korisnika():
    ...
    cursor = db.cursor()
    q = '''INSERT INTO
    korisnici(korisnicko_ime, lozinka, profilna_slika, ime, prezime, pol, adresa, email, telefon, stanje_racuna, prijavljen)
    VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'''
    cursor.execute(q, (data["korisnicko_ime"], data["lozinka"], data["profilna_slika"],
                       data["ime"], data["prezime"], data["pol"], data["adresa"],
...
def izmeni_korisnika(id_korisnika):
    ...
    ime=%s, prezime=%s, pol=%s, adresa=%s,
